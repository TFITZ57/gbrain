 ...
 *
 * In-process unix-socket servers plus one bounded cross-process secret race;
 * hermetic in-memory PGLite for the assembly happy path.
 */
import {
  describe,
  test,
...
describe("turn_context over a real socket", () => {
  test("happy path: pointers + world facts assembled; private facts NEVER cross [S3#1]", async () => {
    ...
    await engine.insertFact(
      {
        fact: "WORLD-FACT alice-example prefers async updates",
        kind: "fact",
    ...
    await engine.insertFact(
      {
        fact: "PRIVATE-FACT secret diligence detail",
        kind: "fact",
        entity_slug: "people/alice-example",
        source: "test",
        visibility: "private",
    ...
    const resp = await requestTurnContext(sock, {
      secret,
      window: [
        { role: "user", text: "met Alice Example today about the round" },
      ],
    });
  ...
  test("cross-source request rejected [CX2-10]", async () => {
    ...
    const resp = await requestTurnContext(sock, {
      secret,
      window: [],
      sourceId: "other-team-brain",
    });
    const r = resp as TurnContextResponse;
    expect(r.ok).toBe(false);
    expect(r.error).toBe("source_mismatch");
    // Matching source claim passes.
    const okResp = await requestTurnContext(sock, {
      secret,
      window: [],
      sourceId: "default",
    });
  ...
  test("request protocol !== 2 → unsupported_protocol", async () => {
    ...
    const resp = (await rawRequest(sock, JSON.stringify(req))) as {
      ok: boolean;
      error?: string;
    };
    expect(resp.ok).toBe(false);
...
describe("stale-serve detection [A9]", () => {
  ...
  test("absent socket → IPC_UNAVAILABLE (fail-soft, mirrors resolve client)", async () => {
    const dir = tmpDir();
    const resp = await requestTurnContext(resolveSocketPath(dir), {
      secret: "x",
      window: [],
    });
...
describe("request clamping [G11]", () => {
  test("oversized window is trimmed oldest-first below the 256KB cap", async () => {
    ...
    const resp = await requestTurnContext(
      sock,
      { secret, window: turns },
      { timeoutMs: 5000 },
    );
    ...
    expect(received[received.length - 1].text.startsWith("turn-599 ")).toBe(
      true,
    );
...
describe("socket + secret hardening [S3#6]", () => {
  ...
  test("concurrent first writers converge on one atomically published secret", async () => {
    ...
    const program = [
      `import { ensureIpcSecret } from ${JSON.stringify(modulePath)};`,
      `process.stdout.write(ensureIpcSecret(${JSON.stringify(dir)}));`,
    ].join("\n");
    ...
    expect(readIpcSecret(dir)).toBe(secrets[0]);
  });
...
describe("server self-budget [G11]", () => {
  test("handler slower than the 1800ms budget degrades to server_budget", async () => {
    const dir = tmpDir();
    const sock = resolveSocketPath(dir);
    const secret = ensureIpcSecret(dir);
    const server = await startResolveIpcServer(
      sock,
      {
        resolve: async () => null,
        turn_context: () =>
          new Promise<TurnContextResult>((r) =>
            setTimeout(() => r(stubBlock), 2000),
          ),
      },
      { secret, boundSourceId: "default" },
    );
    servers.push(server!);
    // Wide client timeout on purpose: the SERVER's own 1800ms budget is what's
...
describe("resolve kind honors boundSourceId [CX2-10]", () => {
  ...
  test("bound server accepts the matching source claim and absent sourceId", async () => {
    ...
    const absent = await resolveViaIpc(sock, {
      candidates: [{ display: "A", query: "A" }],
    });
...
describe("onTurnContextDelivered — the hook lane feedback-loop seam (#2095)", () => {
    ...
    volunteered: [
      {
        ...
        confidence: 0.85,
        arm: "title",
        rationale: 'exact title match "Acme"',
        synopsis: "y",
      },
    ],
    factsCount: 0,
    ...
    const server = await startResolveIpcServer(
      ...
      { resolve: async () => null, turn_context: async () => richBlock },
      {
        ...
        onTurnContextDelivered: (result, req) =>
          delivered.push({ result, req }),
      },
    );
    servers.push(server!);
    const resp = await requestTurnContext(sock, {
      secret,
      window: [{ role: "user", text: "hi Acme" }],
      channel: "claude-code",
      sessionId: "s-1",
    });
  ...
  test("does NOT fire on rejections or empty blocks (nothing was injected)", async () => {
    ...
    const server = await startResolveIpcServer(
      ...
      {
        resolve: async () => null,
        turn_context: async () => (emptyMode ? stubBlock : richBlock),
      },
      {
        secret,
        boundSourceId: "default",
        onTurnContextDelivered: (_r, req) => {
          fired.push(req.sessionId ?? "?");
        },
      },
    );
    servers.push(server!);
  ...
  test("clamp priority: oversized priorContextText is dropped BEFORE any window turn (red-team trim inversion)", async () => {
    ...
    servers.push(server!);
    const window: WindowTurn[] = Array.from({ length: 4 }, (_, i) => ({
      role: "user" as const,
      text: `turn ${i} about Acme`,
    }));
    const resp = await requestTurnContext(sock, {
      secret,
      window,
